...
<hgroup itemprop="name" class="entry-title"><h2 itemprop="headline"><?php echo $yellow->page->getHtml("titleContent") ?></h2></hgroup>
  <?php if($yellow->page->isExisting("image")): ?>
  <div class="none" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
    <figure>
	<img itemprop="url" src="<?php echo $this->yellow->page->getUrl("image") ?>" />
...
<span itemprop="name"><?php $authorCounter = 0; foreach(preg_split("/\s*,\s*/", $yellow->page->get("author")) as $author) { if(++$authorCounter>1) echo ", "; echo "<a href=\"".$yellow->page->getPage("wiki")->getLocation(true).$yellow->toolbox->normaliseArgs("author:$author")."\">".htmlspecialchars($author)."</a>"; } ?></span></span>
</div>
</header>
</div>
...
<div class="author">
 ...
 <span class="avatar">
 <figure>
 <img itemprop="image" src="<?php echo $this->yellow->text->getHtml("photo") ?>" alt="Photo of <?php echo $yellow->page->getHtml("author") ?>"/></figure></span>
       <span class="authordescription" itemprop="description"><?php echo $this->yellow->text->getHtml("authorDescription") ?></span>
...
<span class="meta-tags"><?php echo $yellow->text->getHtml("wikiTag") ?> <?php $tagCounter = 0; foreach(preg_split("/\s*,\s*/", $yellow->page->get("tag")) as $tag) { if(++$tagCounter>1) echo ", "; echo "<span itemprop=\"headline\"><a href=\"".$yellow->page->getPage("wiki")->getLocation(true).$yellow->toolbox->normaliseArgs("tag:$tag")."\">".htmlspecialchars($tag)."</a></span>"; } ?></span>
